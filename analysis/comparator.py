...
class TrigramSimilarity:
    ...
    def compare(
        ...
        scores = pd.Series(self.run_comparison(present, others))
        dev = scores.std()
    ...
    def run_comparison(
        ...
        res = {}
        for other in others:
            quotient = self.similarity(present.comparandum(), other.comparandum())
            res[other] = quotient
        return res
    ...
    @staticmethod
    def find_ngrams(text: str, size: int = 3) -> set:
        ngrams: Set[str] = set()
        if not text:
            return ngrams
